import i18n from "i18next";
import { initReactI18next } from "react-i18next";

const resources = {
  en: {
    translation: {
      Pages: "Pages",
      Sidebar: "Sidebar",
      "Dark mode": "Dark mode",
      "Light mode": "Light mode",
      "Change language": "Change language",
      Cencle: "Cencle",
      Save: "Save",
      Dashboard: "Dashboard",
      Groups: "Groups",
      Students: "Students",
      Attendance: "Attendance",
      Payments: "Payments",
      "Overall statistics and status": "Overall statistics and status",
      "Today's present": "Today's present",
      "Paid students": "Paid students",
      "Students in groups": "Students in groups",
      "Top groups": "Top groups",
      "Attendance today": "Attendance today",
      "Present/Late/Absent": "Present/Late/Absent",
      "Payments status": "Payments status",
      "Paid vs Unpaid": "Paid vs Unpaid",
      Total: "Total",
      Percent: "Percent",
      "No data": "No data",
      "No results found": "No results found",
      "Track and manage payments": "Track and manage payments",
      "Finance overview": "Finance overview",
      "Smart tracking for income, expenses, and student status":
        "Smart tracking for income, expenses, and student status",
      "Total payments": "Total payments",
      "Unpaid students": "Unpaid students",
      "Total revenue": "Total revenue",
      Revenue: "Revenue",
      "Revenue trend": "Revenue trend",
      Monthly: "Monthly",
      "Monthly revenue": "Monthly revenue",
      "Income flow": "Income flow",
      "Search by student or group": "Search by student or group",
      "Add payment": "Add payment",
      All: "All",
      "Student payments": "Student payments",
      "Student status": "Student status",
      "Paid / unpaid": "Paid / unpaid",
      Fee: "Fee",
      Lessons: "Lessons",
      Note: "Note",
      Action: "Action",
      "No students found": "No students found",
      "No payments found": "No payments found",
      "No revenue yet": "No revenue yet",
      Amount: "Amount",
      Date: "Date",
      "Amount placeholder": "e.g. 300000",
      Saving: "Saving...",
      Close: "Close",
      "Select student": "Select student",
      Unknown: "Unknown",
      Cashflow: "Cashflow",
      "Income vs expenses": "Income vs expenses",
      Income: "Income",
      Expenses: "Expenses",
      Net: "Net",
      "Expenses breakdown": "Expenses breakdown",
      "Center costs": "Center costs",
      "Payment rules": "Payment rules",
      Policy: "Policy",
      "First payment is 200,000 so'm": "First payment is 200,000 so'm",
      "Next months are 300,000 so'm": "Next months are 300,000 so'm",
      "New students pay after 4 lessons": "New students pay after 4 lessons",
      "Recent payments": "Recent payments",
      Latest: "Latest",
      "No expenses found": "No expenses found",
      "Expense name": "Expense name",
      "Add expense": "Add expense",
      "All months": "All months",
      Month: "Month",
      Edit: "Edit",
      Delete: "Delete",
      Update: "Update",
      Paid: "Paid",
      Unpaid: "Unpaid",
      Overdue: "Overdue",
      Pending: "Pending",
      "Payment day: before the 7th": "Payment day: before the 7th",
      "Pay after 4 lessons": "Pay after 4 lessons",
      "Payment after 4 lessons": "Payment after 4 lessons",
      "Lessons count": "{{count}} lessons",
      "Attendance History": "Attendance History",
      "Daily Attendance": "Daily Attendance",
      "Attendance  Statistics": "Attendance  Statistics",
      Geo: "Geo",
      Devices: "Devices",
      Loyalty: "Loyalty",
      "Visit Depth": "Visit Depth",
      "Back to Dashboard": "Back to Dashboard",
      "Page Not Found": "Page Not Found",
      "Create group": "Create group",
      "Add student": "Add student",
      "Export excel": "Export excel",
      Create: "Create",
      "Group name": "Group name",
      Weekdays: "Weekdays",
      "Every day": "Every day",
      Monday: "Monday",
      Tuesday: "Tuesday",
      Wednesday: "Wednesday",
      Thursday: "Thursday",
      Friday: "Friday",
      Saturday: "Saturday",
      Sunday: "Sunday",
      "Lesson time": "Lesson time",
      "Select week days": "Select week days",
      "Group name cannot be empty": "Group name cannot be empty",
      "Lesson time cannot be empty": "Lesson time cannot be empty",
      "Please select the lesson days": "Please select the lesson days",
      "Group created successfully!": "Group created successfully!",
      "Failed to create group": "Failed to create group!",
      "Student name": "Student name",
      "Student lastName": "Student lastName",
      "Select group": "Select group",
      "A group with this name already exists!":
        "A group with this name already exists!",
      "No such group found": "No such group found!",
      "An error occurred": "An error occurred:",
      "Student first name must not be empty!":
        "Student first name must not be empty!",
      "Student last name must not be empty!":
        "Student last name must not be empty!",
      "No group selected!": "No group selected!",
      "No weekdays selected!": "No weekdays selected!",
      "Student added successfully!": "Student added successfully!",
      "Group not found!": "Group not found!",
      "Student phoneNumber": "Student phoneNumber",
      "Enter the student's phone number!": "Enter the student's phone number!",
      "Students List": "Students List",
      search_placeholder: "Search by name...",
      phone: "Phone Number",
      actions: "Actions",
      edit_student_title: "Edit Student Information",
      delete_confirm_title: "Are you sure you want to delete?",
      delete_confirm_desc:
        "This action cannot be undone. All student data will be permanently removed.",
      save_button: "Save Changes",
      delete_button: "Delete Student",
      cancel_button: "Cancel",
      change_group: "Move to Another Group",
      error_loading_data: "Failed to load students data",
      student_updated_successfully: "Information updated successfully",
      student_moved_successfully: "Student successfully moved to the new group",
      student_deleted_successfully: "Student record deleted successfully",
      error_deleting_student: "Error occurred while deleting",
      error_occurred: "An unexpected error occurred",
      "Mark attendance": "Mark attendance",
      Check: "Check",
      Today: "Today",
      "Attendance saved": "Attendance saved",
      Came: "Came",
      "Didn't come": "Didn't come",
      Late: "Late",
      Status: "Status",
      Actions: "Actions",
      "There are no students in the group":
        "There are no students in the group",
      "Select group to attend": "Select a group from the list above to attend",
      "Delay time": "Delay time",
      "Enter the tardy time for the following student":
        "Enter the tardy time for the following student:",
      "How many minutes was the student late?":
        "How many minutes was the student late?",
      "Enter the tardy time for the following student:":
        "Enter the tardy time for the following student:",
      Student: "Student",
      Minute: "Minute",
      Confirmation: "Confirmation",
      "Not selected": "Not selected",
      "An error occurred while loading data!":
        "An error occurred while loading data!",
      "Attendance has already been recorded!":
        "Attendance has already been recorded!",
      "Mark the attendance of at least one student!":
        "Mark the attendance of at least one student!",
      "Today's attendance has already been saved!":
        "Today's attendance has already been saved!",
      "Please select a group first!": "Please select a group first!",
      "Attendance saved successfully!": "Attendance saved successfully!",
      "An error occurred while saving attendance!":
        "An error occurred while saving attendance!",
      Group: "Group",
      "Class time": "Class time",
      Day: "Day",
      Saved: "Saved",
      Save: "Save",
      "No phone number": "No phone number",
      Login: "Login",
      "Log in to your account": "Log in to your account",
      "Email address": "Email address",
      Password: "Password",
      "Enter your password": "Enter your password",
      "Do you have an account?": "Do you have an account?",
      "You have successfully logged in": "You have successfully logged in!",
      "Incorrect email or password": "Incorrect email or password",
      "Create account": "Create account",
      "Email verified successfully": "Email verified successfully",
      "Create a new account": "Create a new account",
      "Please fill all fields": "Please fill all fields",
      "Passwords do not match": "Passwords do not match",
      "Verification link has been sent to your email":
        "Verification link has been sent to your email",
      "Registration failed": "Registration failed",
      "Full name": "Full name",
      "Enter your name": "Enter your name",
      "Enter password": "Enter password",
      "Repeat password": "Repeat password",
      "Already have an account?": "Already have an account?",
      Cancel: "Cancel",
      "Continue with Google": "Continue with Google",
      Present: "Present",
      Loading: "Loading...",
      DAILY: "DAILY",
      ATTENDANCE: "ATTENDANCE",
      "Group Attendance & Analysis": "Group Attendance & Analysis",
      "Search group": "Search group...",
      "Graphic statics": "Graphic statics",
      Absent: "Absent",
      Late: "Late",
      Overview: "Overview",
      "Last name not entered": "Last name not entered",
      "No name entered": "No name entered",
      "Attendance Status": "Attendance Status",
      "Take attendance": "Take attendance",
      "Select a group and record students' attendance":
        "Select a group and record students' attendance",
      "Attendance not selected": "Attendance not selected",
    },
  },

  uz: {
    translation: {
      Pages: "Sahifalar",
      Sidebar: "Yon panel",
      "Dark mode": "Tungi rejim",
      "Light mode": "Kunduzgi rejim",
      "Change language": "Tilni almashtirish",
      Cencle: "Bekor qilish",
      Save: "Saqlash",
      Dashboard: "Boshqaruv paneli",
      Groups: "Guruhlar",
      Students: "O'quvchilar",
      Attendance: "Davomat",
      Payments: "To'lovlar",
      "Overall statistics and status": "Umumiy statistika va holat",
      "Today's present": "Bugungi kelganlar",
      "Paid students": "To'lov qilgan",
      "Students in groups": "Guruhlardagi o'quvchilar",
      "Top groups": "Top guruhlar",
      "Attendance today": "Davomat bugun",
      "Present/Late/Absent": "Keldi/Kechikdi/Kelmagan",
      "Payments status": "To'lovlar holati",
      "Paid vs Unpaid": "To'langan vs To'lanmagan",
      Total: "Jami",
      Percent: "Foiz",
      "No data": "Ma'lumot yo'q",
      "No results found": "Natija topilmadi",
      "Track and manage payments": "To'lovlarni nazorat qilish va kuzatish",
      "Total revenue": "Jami tushum",
      "Search by student or group": "O'quvchi yoki guruh bo'yicha qidirish",
      "Add payment": "To'lov qo'shish",
      All: "Barchasi",
      "Student payments": "O'quvchilar to'lovi",
      Lessons: "Darslar",
      Note: "Izoh",
      Action: "Amal",
      "No students found": "O'quvchilar topilmadi",
      "No payments found": "To'lovlar topilmadi",
      Amount: "Summa",
      Date: "Sana",
      "Amount placeholder": "Masalan: 300000",
      Saving: "Saqlanmoqda...",
      Close: "Yopish",
      "Select student": "O'quvchi tanlang",
      Unknown: "Noma'lum",
      Paid: "To'langan",
      Unpaid: "To'lanmagan",
      Overdue: "Muddat o'tgan",
      Pending: "Kutilmoqda",
      "Payment day: before the 7th": "To'lov kuni: har oyning 7-sanasi",
      "Pay after 4 lessons": "4-darsdan keyin to'lov kerak",
      "Payment after 4 lessons": "4-darsdan keyin to'lov",
      "Lessons count": "{{count}} ta",
      "Attendance History": "Davomat tarixi",
      "Daily Attendance": "Kunlik davomat",
      "Attendance  Statistics": "Davomat statistikasi",
      Geo: "Geo",
      Devices: "Qurilmalar",
      Loyalty: "Sodiqlik",
      "Visit Depth": "Tashrif chuqurligi",
      "Back to Dashboard": "Boshqaruv panelga qaytish",
      "Page Not Found": "Sahifa topilmadi",
      "Create group": "Yangi guruh yaratish",
      "Add student": "Yangi o'quvchi qo'shish",
      "Export excel": "Excelga yuklash",
      Create: "Yaratish",
      "Group name": "Guruh nomi",
      Weekdays: "Hafta kunlari",
      "Every day": "Har kuni",
      Monday: "Dushanba",
      Tuesday: "Seshanba",
      Wednesday: "Chorshanba",
      Thursday: "Payshanba",
      Friday: "Juma",
      Saturday: "Shanba",
      Sunday: "Yakshanba",
      "Lesson time": "Dars vaqti",
      "Select week days": "Hafta kunlarini tanlang",
      "Group name cannot be empty": "Guruh nomi bo'sh bo'lmasin",
      "Lesson time cannot be empty": "Dars vaqti bo'sh bo'lmasligi kerak",
      "Please select the lesson days": "Iltimos dars kunlarini tanlang",
      "Group created successfully!": "Guruh muvaffaqiyatli yaratildi!",
      "Failed to create group": "Guruh yaratib bo'lmadi!",
      "Student name": "O'quvchi ismi",
      "Student lastName": "O'quvchi familiyasi",
      "Select group": "Guruhni tanlang",
      "A group with this name already exists!":
        "Bunday nomli guruh allaqachon mavjud!",
      "No such group found": "Bunday guruh mavjud emas!",
      "An error occurred": "Xatolik yuz berdi:",
      "Student first name must not be empty!": "O'quvchi ismi bo'sh bo'lmasin!",
      "Student last name must not be empty!":
        "O'quvchi familiyasi bo'sh bo'lmasin!",
      "No group selected!": "Guruh tanlanmagan!",
      "No weekdays selected!": "Hafta kunlari tanlanmagan!",
      "Student added successfully!": "O'quvchi muvaffaqiyatli qo'shildi!",
      "Group not found!": "Guruh topilmadi!",
      "Student phoneNumber": "O'quvchi telefon raqami",
      "Enter the student's phone number!":
        "O'quvchi telefon raqamini kiriting!",
      "Students List": "O'quvchilar ro'yxati",
      search_placeholder: "Ism bo'yicha qidirish...",
      phone: "Telefon raqami",
      actions: "Amallar",
      edit_student_title: "O'quvchi ma'lumotlarini tahrirlash",
      delete_confirm_title: "Haqiqatan ham o'chirmoqchimisiz?",
      delete_confirm_desc:
        "Bu amalni ortga qaytarib bo'lmaydi. O'quvchi butunlay o'chiriladi.",
      save_button: "Saqlash",
      delete_button: "O'chirish",
      cancel_button: "Bekor qilish",
      change_group: "Boshqa guruhga ko'chirish",
      error_loading_data: "Ma'lumotlarni yuklashda xatolik yuz berdi",
      student_updated_successfully: "Ma'lumotlar muvaffaqiyatli yangilandi",
      student_moved_successfully:
        "O'quvchi yangi guruhga muvaffaqiyatli ko'chirildi",
      student_deleted_successfully: "O'quvchi muvaffaqiyatli o'chirildi",
      error_deleting_student: "O'chirishda xatolik yuz berdi",
      error_occurred: "Kutilmagan xatolik yuz berdi",
      "Mark attendance": "Davomat qlish",
      Cancel: "Bekor qlish",
      Check: "Tanlash",
      Today: "Bugun",
      "Attendance saved": "Davomat saqlangan",
      Came: "Keldi",
      "Didn't come": "Kelmadi",
      Late: "Late",
      Status: "Holati",
      Actions: "Amallar",
      "There are no students in the group": "Guruhda talabalar yo'q",
      "Select group to attend":
        "Davomat qilish uchun yuqoridagi ro'yxatdan guruhni tanlang",
      "Delay time": "Kechikish vaqti",
      "Enter the tardy time for the following student":
        "Quyidagi talaba uchun kechikish vaqtini kiriting:",
      "How many minutes was the student late?": "Talaba necha daqiqa kechikdi?",
      "Enter the tardy time for the following student:":
        "Quyidagi talaba uchun kechikish vaqtini kiriting:",
      Student: "Talaba",
      Minute: "Daqiqa",
      Confirmation: "Tasdiqlash",
      "Not selected": "Tanlanmagan",
      "An error occurred while loading data!":
        "Ma'lumotlarni yuklashda xatolik yuz berdi!",
      "Attendance has already been recorded!": "Davomat allaqachon olingan!",
      "Mark the attendance of at least one student!":
        "Kamida bitta talabaning davomatini belgilang!",
      "Today's attendance has already been saved!":
        "Bugungi davomat allaqachon saqlangan!",
      "Please select a group first!": "Iltimos, avval guruhni tanlang!",
      "Attendance saved successfully!": "Davomat muvaffaqiyatli saqlandi!",
      "An error occurred while saving attendance!":
        "Davomatni saqlashda xatolik yuz berdi!",
      Group: "Guruh",
      "Class time": "Dars vaqti",
      Day: "Kun",
      Saved: "Saqlangan",
      Save: "Saqlash",
      "No phone number": "Telefon raqam yo'q",
      Login: "Tizimga krish",
      "Log in to your account": "Hisobingizga kiring",
      "Email address": "Email manzil",
      Password: "Parol",
      "Enter your password": "Parolingizni kriting",
      "Do you have an account?": "Hisobingiz yo'qmi ?",
      "You have successfully logged in":
        "Siz tizimga muvoffaqiyatli kirdingiz!",
      "Incorrect email or password": "Noto'g'ri elektron pochta yoki parol",
      "Create account": "Hisob yaratish",
      "Email verified successfully":
        "Elektron pochta muvaffaqiyatli tasdiqlandi",
      "Create a new account": "Yangi hisob yarating",
      "Please fill all fields": "Iltimos, barcha maydonlarni to'ldiring",
      "Passwords do not match": "Parollar mos kelmaydi",
      "Verification link has been sent to your email":
        "Tasdiqlash havolasi elektron pochtangizga yuborildi",
      "Registration failed": "Ro'yxatdan o'tish amalga oshmadi",
      "Full name": "To'liq ism",
      "Enter your name": "Ismingizni kiriting",
      "Enter password": "Parolni kiriting",
      "Repeat password": "Parolni qayta kiriting",
      "Already have an account?": "Hisobingiz bormi?",
      "Continue with Google": "Google bilan bog'lash",
      Present: "Keldi",
      Loading: "Yuklanmoqda...",
      DAILY: "KUNLIK",
      ATTENDANCE: "DAVOMAT",
      "Group Attendance & Analysis": "Guruhlar davomati va tahlili",
      "Search group": "Guruh izlash...",
      "Graphic statics": "Grafik statika",
      Absent: "Kelmadi",
      Late: "Kechikdi",
      Overview: "Umumiy ko'rish",
      "Last name not entered": "Familiya kiritilmagan",
      "No name entered": "Ism kritilmagan",
      "Attendance Status": "Davomat holati",
      "Take attendance": "Davomat qlish",
      "Select a group and record students' attendance":
        "Guruh tanlab, talabalarning davomatini belgilang ",
      "Attendance not selected": "Davomat tanlanmagan",
    },
  },

  ru: {
    translation: {
      Pages: "Страницы",
      Sidebar: "Боковая панель",
      "Dark mode": "Тёмный режим",
      "Light mode": "Светлый режим",
      "Change language": "Сменить язык",
      Cencle: "Отмена",
      Save: "Сохранить",
      Dashboard: "Панель управления",
      Groups: "Группы",
      Students: "Студенты",
      Attendance: "Посещаемость",
      Payments: "Платежи",
      "Overall statistics and status": "Общая статистика и состояние",
      "Today's present": "Пришли сегодня",
      "Paid students": "Оплатили",
      "Students in groups": "Ученики по группам",
      "Top groups": "Топ группы",
      "Attendance today": "Посещаемость сегодня",
      "Present/Late/Absent": "Пришел/Опоздал/Отсутствовал",
      "Payments status": "Статус платежей",
      "Paid vs Unpaid": "Оплачено vs Не оплачено",
      Total: "Итого",
      Percent: "Процент",
      "No data": "Нет данных",
      "No results found": "Ничего не найдено",
      "Track and manage payments": "Контроль и управление платежами",
      "Total revenue": "Общая сумма",
      "Search by student or group": "Поиск по ученику или группе",
      "Add payment": "Добавить платеж",
      All: "Все",
      "Student payments": "Платежи учеников",
      Lessons: "Уроки",
      Note: "Примечание",
      Action: "Действие",
      "No students found": "Ученики не найдены",
      "No payments found": "Платежи не найдены",
      Amount: "Сумма",
      Date: "Дата",
      "Amount placeholder": "Напр.: 300000",
      Saving: "Сохранение...",
      Close: "Закрыть",
      "Select student": "Выберите ученика",
      Unknown: "Неизвестно",
      Paid: "Оплачено",
      Unpaid: "Не оплачено",
      Overdue: "Просрочено",
      Pending: "Ожидание",
      "Payment day: before the 7th": "Оплата до 7-го числа",
      "Pay after 4 lessons": "Оплата после 4 уроков",
      "Payment after 4 lessons": "Оплата после 4 уроков",
      "Lessons count": "{{count}} уроков",
      "Attendance History": "История посещаемости",
      "Daily Attendance": "Ежедневная посещаемость",
      "Attendance  Statistics": "Статистика посещаемости",
      Geo: "Гео",
      Devices: "Устройства",
      Loyalty: "Лояльность",
      "Visit Depth": "Глубина визита",
      "Back to Dashboard": "Назад к панели",
      "Page Not Found": "Страница не найдена",
      "Create group": "Создать группу",
      "Add student": "Добавить студента",
      "Export excel": "Экспорт в Excel",
      Create: "Создать",
      "Group name": "Название группы",
      Weekdays: "Дни недели",
      "Every day": "Каждый день",
      Monday: "Понедельник",
      Tuesday: "Вторник",
      Wednesday: "Среда",
      Thursday: "Четверг",
      Friday: "Пятница",
      Saturday: "Суббота",
      Sunday: "Воскресенье",
      "Lesson time": "Время занятия",
      "Select week days": "Выберите дни недели",
      "Group name cannot be empty": "Название группы не может быть пустым",
      "Lesson time cannot be empty": "Время занятия не может быть пустым",
      "Please select the lesson days": "Пожалуйста, выберите дни занятий",
      "Group created successfully!": "Группа успешно создана!",
      "Failed to create group": "Не удалось создать группу!",
      "Student name": "Имя студента",
      "Student lastName": "Фамилия студента",
      "Select group": "Выберите группу",
      "A group with this name already exists!":
        "Группа с таким названием уже существует!",
      "No such group found": "Такая группа не найдена!",
      "An error occurred": "Произошла ошибка:",
      "Student first name must not be empty!":
        "Имя студента не должно быть пустым!",
      "Student last name must not be empty!":
        "Фамилия студента не должна быть пустой!",
      "No group selected!": "Группа не выбрана!",
      "No weekdays selected!": "Дни недели не выбраны!",
      "Student added successfully!": "Студент успешно добавлен!",
      "Group not found!": "Группа не найдена!",
      "Student phoneNumber": "Телефон студента",
      "Enter the student's phone number!": "Введите номер телефона студента!",
      "Students List": "Список студентов",
      search_placeholder: "Поиск по имени...",
      phone: "Телефон",
      actions: "Действия",
      edit_student_title: "Редактировать данные студента",
      delete_confirm_title: "Вы уверены, что хотите удалить?",
      delete_confirm_desc:
        "Это действие нельзя отменить. Все данные будут удалены.",
      save_button: "Сохранить",
      delete_button: "Удалить",
      cancel_button: "Отмена",
      change_group: "Перевести в другую группу",
      error_loading_data: "Ошибка при загрузке данных студентов",
      student_updated_successfully: "Данные успешно обновлены",
      student_moved_successfully: "Студент успешно переведён в новую группу",
      student_deleted_successfully: "Студент успешно удалён",
      error_deleting_student: "Ошибка при удалении студента",
      error_occurred: "Произошла непредвиденная ошибка",
      "Mark attendance": "Mark attendance",
      Check: "Выбрать",
      Today: "Сегодня",
      "Attendance saved": "Посещаемость сохранена",
      Came: "Пришел(а)",
      "Didn't come": "Не пришел(а)",
      Late: "Опоздал(а)",
      Status: "Статус",
      Actions: "Действия",
      "There are no students in the group": "В группе нет студентов",
      "Select group to attend":
        "Выберите группу из списка выше, чтобы отметить посещаемость",
      "Delay time": "Время опоздания",
      "Enter the tardy time for the following student":
        "Введите время опоздания для следующего студента:",
      "How many minutes was the student late?":
        "На сколько минут опоздал студент?",
      "Enter the tardy time for the following student:":
        "Введите время опоздания для следующего студента:",
      Student: "Студент",
      Minute: "Минута",
      Confirmation: "Подтверждение",
      "Not selected": "Не выбрано",
      "An error occurred while loading data!":
        "Произошла ошибка при загрузке данных!",
      "Attendance has already been recorded!": "Посещаемость уже отмечена!",
      "Mark the attendance of at least one student!":
        "Отметьте посещаемость хотя бы одного студента!",
      "Today's attendance has already been saved!":
        "Сегодняшняя посещаемость уже сохранена!",
      "Please select a group first!": "Пожалуйста, сначала выберите группу!",
      "Attendance saved successfully!": "Посещаемость успешно сохранена!",
      "An error occurred while saving attendance!":
        "Произошла ошибка при сохранении посещаемости!",
      Group: "Группа",
      "Class time": "Время занятий",
      Day: "День",
      Saved: "Сохранено",
      Save: "Сохранить",
      "No phone number": "Номер телефона отсутствует",
      Login: "Вход",
      "Log in to your account": "Войдите в свой аккаунт",
      "Email address": "Адрес электронной почты",
      Password: "Пароль",
      "Enter your password": "Введите ваш пароль",
      "Do you have an account?": "У вас есть аккаунт?",
      "You have successfully logged in": "Вы успешно вошли в систему!",
      "Incorrect email or password": "Неверный email или пароль",
      "Create account": "Создать аккаунт",
      "Email verified successfully": "Электронная почта успешно подтверждена",
      "Create a new account": "Создать новый аккаунт",
      "Please fill all fields": "Пожалуйста, заполните все поля",
      "Passwords do not match": "Пароли не совпадают",
      "Verification link has been sent to your email":
        "Ссылка для подтверждения отправлена на вашу электронную почту",
      "Registration failed": "Регистрация не удалась",
      "Full name": "Полное имя",
      "Enter your name": "Введите имя",
      "Enter password": "Введите пароль",
      "Repeat password": "Повторите пароль",
      "Already have an account?": "Уже есть аккаунт?",
      Cancel: "Отмена",
      "Continue with Google": "Войти через Google",
      Present: "Присутствует",
      Loading: "Загрузка...",
      DAILY: "ЕЖЕДНЕВНО",
      ATTENDANCE: "ПОСЕЩАЕМОСТЬ",
      "Group Attendance & Analysis": "Посещаемость и анализ группы",
      "Search group": "Поиск группы...",
      "Graphic statics": "Графическая статистика",
      Absent: "Отсутствует",
      Late: "Опоздал",
      Overview: "Обзор",
      "Last name not entered": "Фамилия не введена",
      "No name entered": "Имя не введено",
      "Attendance Status": "Статус посещаемости",
      "Take attendance": "Отметить посещаемость",
      "Select a group and record students' attendance":
        "Выберите группу и отметьте посещаемость студентов",
      "Attendance not selected": "Явка не выбрана",
    },
  },
};

i18n.use(initReactI18next).init({
  resources,
  lng: localStorage.getItem("Language") || "en",
  interpolation: {
    escapeValue: false,
  },
});

export default i18n;
